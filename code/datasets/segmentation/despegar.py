...
class _Despegar(data.Dataset):
    """Base class
    This contains fields and methods common to all Despegar datasets
    """

    def __init__(self, config=None, split=None, purpose=None, preload=False):
        super(_Despegar, self).__init__()

        self.split = split  # all
        self.purpose = purpose  # train or test

        self.root = config.dataset_root

        self.single_mode = hasattr(config, "single_mode") and config.single_mode

        # assert (os.path.exists(os.path.join(self.root, "debugged.out"))) # No need for debugged.out in Despegar dataset
    ...
    def _set_files(self):
        if self.split in ["all"]: #["unlabelled_train", "labelled_train", "labelled_test"]:
    ...
    def _load_data(self, image_id):
        image_path = osp.join(self.root, "train_200", image_id + ".jpg")
        # label_path = osp.join(self.root, "gt", image_id + ".npy")
        import imageio
        ...
        label = np.copy(image)[:,:,1]
        label[label <= 100] = 0
        label[label > 100] = 1
        return image, label
    
"""     def _flatten_gt(self, gt):
...
class Despegar(_Despegar):
    def __init__(self, **kwargs):
        super(Despegar, self).__init__(**kwargs)
        ...
        self._fine_to_coarse_dict = {
            0: 0, 3: 0, 4: 0, 10: 0, # unlabeled, other, pedestrian, car,
            7: 1, # road
            6: 2, 8: 2,  # road line, sidewalk (relevant points)
            11: 3, 1: 3, 2: 3, 5: 3, 12: 3, # wall, building, fence, pole, traffic sign, 
            9: 4  # vegetation, 
            }
